...
document.getElementById('extractButton').addEventListener('click', function () {
    ...
    audioProgress.style.display = 'flex'; // Mostrar la barra de progreso
    ...
    const interval = setInterval(() => {
        progress += 17; // Incrementar 17%
        if (progress >= 100) {
            progress = 100; // Limitar a 100%
            clearInterval(interval); // Detener el intervalo al llegar a 100%
            showSuccessAlert(successAlert); // Mostrar la alerta de éxito
        ...
        progressBar.setAttribute('aria-valuenow', progress); // Actualizar el valor ARIA
